...
stage_pattern = (os.getenv("STAGE_PATTERN") or "").strip()
...
try:
    ...
    if stage_pattern:
        cur.execute(f"REMOVE @{stage_path} PATTERN='{stage_pattern}'")
    else:
        cur.execute(f"REMOVE @{stage_path}")
    cur.execute(f"LIST @{stage_path}")
    remaining = cur.fetchall()
    if remaining:
        print("Stage still has files:")
        for row in remaining:
            print(row)
    else:
        print(f"Cleared stage {stage_path}")
finally:
    cur.close()
    conn.close()
